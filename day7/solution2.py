"""Solution to day 7, part 2 puzzle on adventofcode.com"""
...
def replace_jockers(hand: str) -> str:
    """Replace the jocker with the most common card in the hand."""
    occurences = {x: hand.count(x) for x in hand if x != "J"}
    # Find the most common card in the hand
    if len(occurences) == 0:
        return hand
    most_common = max(occurences, key=occurences.get)
    # Replace the jocker with the most common card
...
def get_ranks(hands: str) -> int:
    ...
    for hand in hands:
        # Replace the jocker with the most common card
        temp_hand = replace_jockers(hand=hand)
        # Find duplicate numbers in string
        duplicates = {x: temp_hand.count(x) for x in temp_hand if temp_hand.count(x) > 1}
        if len(duplicates) == 0:
...
def main_logic(hands: list[str], bids: list[str]) -> int:
    """Main logic for the puzzle. We need to find the total number of winnings."""
